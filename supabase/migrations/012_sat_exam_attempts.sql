-- SAT attempt + answer storage.
-- Clients may SELECT their own rows; INSERT/UPDATE is server-only (service role).
...
drop policy if exists "Users can read own sat exam attempts" on public.sat_exam_attempts;
create policy "Users can read own sat exam attempts"
  on public.sat_exam_attempts for select
  to authenticated
  using (auth.uid() = user_id);

drop policy if exists "Users can read own sat exam answers" on public.sat_exam_answers;
...
-- Match security hardening: browser roles cannot forge scores.
revoke insert, update, delete on public.sat_exam_attempts from anon, authenticated;
revoke insert, update, delete on public.sat_exam_answers from anon, authenticated;
grant select on public.sat_exam_attempts to authenticated;
grant select on public.sat_exam_answers to authenticated;
